        ...
        <div className="bg-card rounded-2xl shadow-lg border p-8">
          ...
          <div className="mt-6 space-y-3">
            <button 
              onClick={async () => {
                ...
                try {
                  ...
                  if (data.url) {
                    ...
                    setError('구글 로그인 설정에 문제가 있습니다');
                    setGoogleLoading(false);
                  }
                } catch (err) {
                  setError(errorMessage(err, '구글 로그인 실패'));
                  setGoogleLoading(false);
                }
              }}
              disabled={googleLoading}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-border rounded-lg hover:bg-muted/50 transition disabled:opacity-50">
              ...
              <span className="text-foreground">{googleLoading ? '연결 중...' : 'Google로 계속하기'}</span>
            </button>
          </div>
