...
- 네이버 블로그 이웃 스크래핑 (Playwright 기반, 모바일/데스크톱 모두 지원)
 ...
 - 이메일 모듈 빌드 오류 수정 (Gmail 발송 라우트 스코프 이슈 해결)
 - 랜딩페이지: 히어로·CTA·가격·리뷰·로고·Bento 시각화·메인 슬라이드 반영
 - 이미지 생성 기능 제거 및 관련 코드 정리
 - Gmail OAuth 실제 연동 테스트 완료
 - 프로젝트별 발송 이력 DB 저장
 - 랜딩페이지: 가격 섹션 기본값 월간으로 변경 (연결제 → 월결제 우선)
 - 배포 빌드 오류 수정: Gmail 라우트 정규식/타입 보완 (컴파일 실패 해결)
 - 관리자: 사용자 관리 라벨 캠페인→프로젝트 교체
 - 사용자 관리: 페이지 크기 선택(10/25/50) 추가
 - 활동 로그 API: campaignId 필터 추가
 - 프로젝트 API: campaignId 포함 반환
 - 프로젝트 모니터링: 카드에 최근 활동 3개 표시
